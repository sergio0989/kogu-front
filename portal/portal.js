...
(function () {
  ...
  async function loadAll() {
    ...
    let exp = {}, reqs = [], envios = [], bancarios = [], cfdis = [], ppdSinRep = [];
    try {
    ...
    render({ expediente, documentos, reqs, envios, nAprob, nRev, nRech, bancarios, cfdis, ppdSinRep });
  }

  let activeTab = 'exp';  // exp | banca | hist | cfdi — persiste entre recargas

  function render({ documentos, reqs, envios, nAprob, nRev, nRech, bancarios = [], cfdis = [], ppdSinRep = [] }) {
    const BANCO_BADGE = {
    ...
    $('app').innerHTML = `
      ...
      <div class="card" style="margin-top:16px${ppdSinRep.length ? ';border:1px solid #fde68a' : ''}">
        ...
        <p class="muted" style="font-size:13px;margin-top:6px">Facturas <strong>PPD</strong> que la empresa ya tiene del SAT y aún <strong>no tienen complemento de pago (REP)</strong> conciliado. Emite y timbra el REP correspondiente.</p>
        ${ppdSinRep.length ? `
        <div class="table-wrap" style="margin-top:10px">
          <table><thead><tr><th>UUID</th><th>Serie/Folio</th><th>Fecha</th><th style="text-align:right">Total</th><th style="text-align:right">Saldo</th><th>Estatus</th></tr></thead>
          <tbody>${ppdSinRep.map(c => `
            <tr>
              ...
              <td style="text-align:right;white-space:nowrap">${c.saldo_pendiente != null ? '$' + Number(c.saldo_pendiente).toLocaleString('es-MX', { minimumFractionDigits: 2 }) : '—'}</td>
              <td>${c.estatus_conciliacion === 'PARCIAL'
                ? '<span class="chip" style="background:#fef3c7;color:#92600c">Parcial</span>'
                : '<span class="chip" style="background:#fee2e2;color:#991b1b">Sin REP</span>'}</td>
            </tr>`).join('')}</tbody></table>
