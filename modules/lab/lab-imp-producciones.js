...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── Estado ──────────────────────────────────────
  const STATE_KEY = 'kogu_lab_imp_prod_state';
  let rows = [];
  ...
  function abrirModalImport() {
    ...
    oQ('#uploadBtn').addEventListener('click', async () => {
      ...
      try {
        ...
        // Filas que ya generaron lote: el reimport NO las pisa, así que una
        // corrección del ERP sobre ellas no entró. Antes pasaba en silencio.
        const yaProcesadas = data?.ya_procesadas ?? 0;
        const hayAdvertencias = (data?.sin_producto?.length ?? 0) > 0 || yaProcesadas > 0;
        ...
        oQ('#importResult').innerHTML = `
          ...
          ${(data?.sin_producto?.length ?? 0) > 0
              ? `<br><span style="font-size:12px">Los lotes con cve_prod sin match importarán sin producto — vincúlalos desde el catálogo de Productos.</span>`
              : ''}
          ${yaProcesadas > 0
              ? `<br><span style="font-size:12px"><strong>${yaProcesadas} fila(s) NO se actualizaron</strong> porque ya generaron su lote.
                 Si el ERP corrigió alguna de ellas, ese cambio no entró.
                 ${(data?.lotes_ya_procesados?.length ?? 0) > 0
                     ? `Lotes: ${escapeHtml(data.lotes_ya_procesados.join(', '))}.` : ''}</span>`
              : ''}
        `;
