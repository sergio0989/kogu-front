...
<div class="card" id="tablaCard" style="margin-top:16px;display:none">
  <div class="row" style="flex-wrap:wrap;gap:10px">
    ...
    <div style="display:flex;gap:8px;align-items:center">
      <input type="text" id="q" class="input" placeholder="Buscar producto / lote…" style="width:200px"/>
  ...
  function pintarKpis(resumen, parametros) {
    ...
    $('kpis').innerHTML = [
      ...
      kpiCard('Anomalías (prod+lote)', fmtNum(pl.anomalias || 0), `${fmtNum(pr.anomalias || 0)} a nivel producto`, (pl.anomalias > 0 ? '#dc2626' : '#16a34a')),
      kpiCard('Dif. existencia (kg)', fmtNum(pl.dif_kg || 0), 'partidas fuera de tolerancia', '#ca8a04'),
      kpiCard('Dif. costo', fmtNum(pl.dif_costo || 0), 'partidas fuera de ±' + fmtPct(parametros ? parametros.var_costo_pct : null), '#ec4899'),
      kpiCard('Costo-cero', fmtNum(pl.costo_cero || 0), 'existencia con costo < piso', '#8b5cf6'),
  ...
  function pintarTabla(rows, meta) {
    ...
    const body = rows.map(r => `<tr style="text-align:right;border-bottom:1px solid #f1f5f9;${r.anomalia ? 'background:#fef2f2' : ''}">
      ...
      <td style="padding:6px;text-align:center">${chipOK(r.dif_existencia_ok, 'OK', '≠')}</td>
    ...
    $('tabla').innerHTML = head + '<tbody>' + (body || `<tr><td colspan="14" class="muted" style="padding:12px;text-align:center">Sin partidas.</td></tr>`) + '</tbody>';
    $('tablaMeta').textContent = `${meta ? meta.total : rows.length} partidas · nivel ${esLote ? 'producto+lote' : 'sumatoria producto'}${soloAnomalias ? ' · solo anomalías' : ''}`;
  }
