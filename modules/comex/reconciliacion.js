  ...
  function renderReconTable() {
    ...
    $('tRecon').innerHTML = head + '<tbody>' + rows.map(r => {
      ...
      return `<tr style="border-bottom:1px solid #f1f5f9;text-align:right${fuera ? ';background:#fffbf5' : ''}">
        ...
        <td style="text-align:left;padding:6px;font-weight:700">${esc(r.pedimento_matriz || r.pedimento || '')}${r.pedimento_matriz && r.pedimento && r.pedimento_matriz !== r.pedimento ? `<div style="font-size:10px;color:#b45309;font-weight:400" title="REFERENCIA capturada en SAI (difiere del pedimento de la matriz SAT)">REF: ${esc(r.pedimento)}</div>` : ''}${r.costeo_folio ? `<div style="font-size:10.5px;color:#64748b;font-weight:400">vs ${esc(r.costeo_folio)} · v${esc(r.costeo_version)}</div>` : ''}</td>
