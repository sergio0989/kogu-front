  ...
  function pintaReales(d) {
    ...
    const lista = `<div style="font-weight:700;color:#334155;font-size:12.5px;margin:12px 0 2px">Operaciones (${n0(ops.length)})</div>
      ...
      <div style="overflow-x:auto"><table class="table" style="width:100%;font-size:12px;font-variant-numeric:tabular-nums"><thead><tr style="border-bottom:1px solid #e2e8f0;text-align:right;color:#64748b;font-size:11px">
        <th style="text-align:left;padding:5px 6px">Pedimento</th><th style="text-align:left;padding:5px 6px">Periodo</th><th>Kg</th><th>Gastos real/kg</th><th>Gastos teó/kg</th><th>Desv</th><th>Dif USD</th><th style="text-align:right;padding:5px 6px">Resultado</th></tr></thead><tbody>
        ${ops.map(o => { const dif = ((Number(o.total_kg) || 0) - (Number(o.teo_kg) || 0)) * (Number(o.kg_total) || 0); return `<tr style="border-bottom:1px solid #f1f5f9;text-align:right"><td style="text-align:left;padding:5px 6px;font-weight:700">${esc(o.pedimento || o.no_costeo)} <span class="muted" style="font-weight:400;font-size:10.5px">v${esc(o.costeo_version || '')}</span></td><td style="text-align:left;padding:5px 6px;color:#64748b">${esc(o.periodo)}</td><td style="padding:5px 6px">${n0(o.kg_total)}</td><td style="padding:5px 6px">$${(Number(o.total_kg) || 0).toFixed(2)}</td><td style="padding:5px 6px;color:#b45309">$${(Number(o.teo_kg) || 0).toFixed(2)}</td><td style="padding:5px 6px;font-weight:700;color:${Number(o.desv_pct) > 0 ? '#991b1b' : '#166534'}">${o.desv_pct == null ? '—' : (Number(o.desv_pct) * 100).toFixed(1) + '%'}</td><td style="padding:5px 6px;font-weight:700;color:${dif > 0 ? '#991b1b' : '#166534'}">${dif < 0 ? '−' : '+'}$${n0(Math.abs(dif))}</td><td style="padding:5px 6px;text-align:right">${rchip(o.resultado)}</td></tr>`; }).join('')}
        ${(() => { const tot = ops.reduce((a, o) => a + ((Number(o.total_kg) || 0) - (Number(o.teo_kg) || 0)) * (Number(o.kg_total) || 0), 0); const tkg = ops.reduce((a, o) => a + (Number(o.kg_total) || 0), 0); return `<tr style="background:#f8fafc;font-weight:800;text-align:right;border-top:2px solid #e2e8f0"><td style="text-align:left;padding:6px">Total (real − teórico)</td><td></td><td style="padding:6px">${n0(tkg)}</td><td colspan="3"></td><td style="padding:6px;color:${tot > 0 ? '#991b1b' : '#166534'}">${tot < 0 ? '−' : '+'}$${n0(Math.abs(tot))}</td><td></td></tr>`; })()}
      </tbody></table></div>`;
